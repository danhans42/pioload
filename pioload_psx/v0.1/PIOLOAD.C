/* PIOLOAD by danhans42
   ...
   Raspberry Pi. Comms is one way only (PC -> PSX)
   
   Version History
   ===============
   
   0.1 - First Version (It works, just loads EXE's thats it)
   
   Future Enhancements (Todo List)
   * Prettier Look                                    
   * Load BIN data to RAM address (TIM/VAG/LIBPS etc)
   * Able to load LIBPS into RAM from CD
   * Load/Execute CPE Executables
   * Load/Execute ECOFF Executables
...
void pcSendByte(char byte)
{
 	char temp;
	temp = ((byte>>6)&3);            	
	*(char *)0x1F060001 = temp;          
	*(char *)0x1F060001 = temp + 8;     
	while (xpGetBit()!=0);              
	temp = (byte>>3)&7;
	*(char *)0x1F060001 = temp + 8;    
	*(char *)0x1F060001 = temp;        
